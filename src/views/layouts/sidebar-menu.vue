                    ...
                    <template v-for="menu in item.menu" :key="menu.menuValue">
                        <li v-if="!menu.hasSubRoute" :class="{ 'active': isMenuActive(menu) }">
                            <router-link v-if="menu.route" :to="menu.route">
                                ...
                                <span v-if="menu.Badge" class="badge badge-danger fs-10 fw-medium text-white p-1">{{
                                    menu.Badge }}</span>
                            </router-link>
                        </li>
                        <li v-else class="submenu">
                            ...
                            <ul :class="menu.showSubRoute ? 'd-block' : 'd-none'">
                                <li v-for="(subMenu, index) in menu.subMenus" :key="index">
                                    <a v-if="subMenu.external && subMenu.url" :href="subMenu.url" target="_blank"
                                        rel="noopener">
                                        {{ subMenu.menuValue }}
                                    </a>
                                    <router-link v-else-if="subMenu.route" :to="subMenu.route">
                                        {{ subMenu.menuValue }}
                                        <span v-if="subMenu.Badge"
                        ...
                        <li v-if="menu.hasSubRouteTwo" class="submenu">
                            ...
                            <ul
                                :class="{ 'd-block': openMenuItem === menu || isActive(menu), 'd-none': openMenuItem !== menu && !isActive(menu) }">
                                <li v-for="subMenus in menu.subMenus" :key="subMenus.menuValue">
                                    <template v-if="!subMenus.customSubmenuTwo">
                                        ...
                                        <router-link v-if="subMenus.route" :to="subMenus.route"
                                            router-link-active="active">{{ subMenus.menuValue }}<span
                                                v-if="subMenus.Badge"
                                                class="badge badge-danger fs-10 fw-medium text-white p-1">{{
                                                    subMenus.Badge
                                                }}</span></router-link>
                                ...
                                <li class="submenu submenu-two">
                                    <a href="javascript:void(0);" @click="openSubmenuOne(subMenus)"
                                        ...
                                        {{ subMenus.menuValue }}<span
                                            class="badge badge-danger fs-10 fw-medium text-white p-1">{{
                                                subMenus.Badge
                                            }}</span><span class="menu-arrow inside-submenu"></span>
                                    </a>
                                    <ul
                                        ...
                                        <li v-for="subMenuTwo in subMenus.subMenusTwo" :key="subMenuTwo.menuValue">
                                            ...
                                            <router-link v-if="subMenuTwo.route" :to="subMenuTwo.route">{{
                                                subMenuTwo.menuValue }}<span v-if="subMenuTwo.Badge"
                                                    class="badge badge-danger fs-10 fw-medium text-white p-1">{{
                                                        subMenuTwo.Badge
                                                    }}</span></router-link>
                                        </li>
...
<li class="submenu-open">
    ...
    <ul>
        <li><a href="https://smarthr.co.in/demo/documentation/vue.html" target="_blank"><i
                    class="ti ti-file-text fs-16 me-2"></i><span>Documentation</span></a></li>
        <li><a href="https://smarthr.co.in/demo/documentation/changelog.html" target="_blank"><i
                    class="ti ti-exchange fs-16 me-2"></i><span>Changelog </span><span
                    class="badge bg-primary badge-xs text-white fs-10 ms-2">v4.1.2</span></a></li>
        <li class="submenu">
            <a href="javascript:void(0);" @click="toggleMultiLevel" :class="{ subdrop: multiLevelOpen }"><i
                    class="ti ti-menu-2 fs-16 me-2"></i><span>Multi Level</span><span class="menu-arrow"></span></a>
            <ul :class="multiLevelOpen ? 'd-block' : 'd-none'">
                <li><a href="javascript:void(0);">Level 1.1</a></li>
                <li class="submenu submenu-two"><a href="javascript:void(0);" @click.stop="toggleLevel12"
                        :class="{ subdrop: level12Open }">Level 1.2<span class="menu-arrow inside-submenu"></span></a>
                    <ul :class="level12Open ? 'd-block' : 'd-none'">
                        <li><a href="javascript:void(0);">Level 2.1</a></li>
                        <li class="submenu submenu-two submenu-three"><a href="javascript:void(0);"
                                @click.stop="toggleLevel22" :class="{ subdrop: level22Open }">Level 2.2<span
    ...
    computed: {
        ...
        isActive() {
            return (menu) => {
                ...
                let base = result[0];
                return base === menu.active_link || base === menu.active_link1 || base === menu.active_link2
            }
        },
        isSubActive() {
            return (menu) => {
